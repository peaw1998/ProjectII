...
export default class TeacherExercise extends Component {
    ...
    test = async () => {
        ...
        let arr = this.state.question.filter(
            (item) => {
                if (item.preTest === true)
                    return true
                else
                    return false
            }
        ...
        let arr2 = this.state.question.filter(
            (item) => {
                if (item.postTest === true)
                    return true
                else
                    return false
            }
    ...
    Get = async () => {
        await axios.get('https://fast-ridge-57035.herokuapp.com/api/exercises/subject/' + this.props.navigation.getParam('_id', 'test'),
            {
                headers: {
